...
- `verify requirement FAF-LLR-123 and generate proof`
...
Expected: the requirement-verification workflow activates.

These prompts are for activation testing only; the actual verification flow still runs through the coordinator-led multi-agent runtime and may use the legacy extraction prompts only as fallback parsing helpers.
...
2. Confirm `LLT Verification` or the current skill name appears in the list.
