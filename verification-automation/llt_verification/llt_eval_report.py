    ...
    def generate_step2_output(self, result: Dict) -> str:
        ...
        if inputs:
            for i, inp in enumerate(inputs, 1):
                ...
                if term_info:
                    ...
                    lines.append(f"{i}. name: {inp} input type: {inp_type_str} min value: {min_val} max value: {max_val}{exact_value_str} found in: {source}")
        ...
        if outputs:
            for i, out in enumerate(outputs, 1):
                ...
                if term_info:
                    ...
                    exact_value = term_info.get("value")
                    exact_value_str = f" exact value: {exact_value}" if exact_value is not None else ""
                    lines.append(f"{i}. name: {out} type: {out_type_str}{exact_value_str} if expression 1 is true (TRUE), Default/ if False(FALSE)")
        ...
        if source_constants:
            ...
            for i, item in enumerate(source_constants, 1):
                lines.append(f"{i}. name: {item.get('name')} type: {item.get('type', 'unknown')} exact value: {item.get('value')} source: {item.get('source', 'source file')}")
        if source_constraints:
            lines += ["", "## Source Constraints"]
            for i, item in enumerate(source_constraints, 1):
                lines.append(f"{i}. expression: {item.get('expression')} source: {item.get('source', 'source file')} line: {item.get('line', '')}")
        return "\n".join(lines)
